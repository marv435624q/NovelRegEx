...
class MainActivity : AppCompatActivity() {
  ...
  override fun onPause() {
    // Keep WebView JavaScript alive only while TTS/binge actually needs it.
  ...
  inner class NovelTtsController : UtteranceProgressListener() {
    ...
    private fun syncPlaybackLocks() {
      ...
      val shouldHoldWifi = active || waitingForNextChapter
    ...
    fun onViewerPageReady() {
      ...
      // Restore the visible playback position after the viewer finishes loading.
    ...
    fun needsBackgroundWebView(): Boolean = active || waitingForNextChapter
    ...
    fun updateEpisodeRange(novelNo: String, currentEpisode: Int, latestEpisode: Int) {
      ...
      // 회차 전환 중 이전 화면에서 늦게 도착한 응답은 현재 재생 회차를
